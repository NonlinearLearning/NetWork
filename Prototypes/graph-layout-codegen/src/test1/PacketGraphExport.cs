...
// The catalog is intentionally explicit: adding a Graph to code generation is
// a source-level registration decision, not assembly scanning or reflection.
public static class PacketGraphCatalog
{
    public static IReadOnlyList<PacketGraphManifest> Export()
    {
        PacketGraphManifest[] manifests =
        [
            new PlayerControlsPacket13GraphConcept().Layout.ExportManifest(),
            new ItemTweakerPacket88GraphConcept().Layout.ExportManifest(),
            new AreaTileChangePacket20GraphConcept().Layout.ExportManifest()
        ];
        return new ReadOnlyCollection<PacketGraphManifest>(manifests.OrderBy(static manifest => manifest.MessageId).ToArray());
    }
}
